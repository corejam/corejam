describe("Canvas Tests", function () {
  ...
  it("Can drag and drop components", function () {
    ...
    cy.getTag("dragger-row")
      .trigger("pointerdown")

    cy.getTag("drop")
      .trigger("pointerover", 50, 0)
      .trigger("pointerup")

    cy.getTag("dragger-headline")
      .trigger("pointerdown")

    cy.getTag("drop").children().first()
    .trigger("pointerover", "center")
    .trigger("pointerup")

    cy.getTag("corejam-builder").click()

    cy.getTag("drop").invoke("html").then(html => {
